...
def trace_lines_and_connections(
    ...
    # E. Map Electrical Panels to Circuits/Equipment (FEEDS)
    if dt in ("ELECTRICAL_LAYOUT", "SLD", "CABLE_SCHEDULE"):
        panel_tags = [p.get("tag") for p in panels if "DB" in p.get("tag", "").upper() or "PANEL" in p.get("tag", "").upper()]
        circuit_tags = [c.get("tag") for c in panels if c.get("tag") not in panel_tags]

        if panel_tags and circuit_tags:
            main_panel = panel_tags[0]
            for ctag in circuit_tags[:20]:
                rkey = (main_panel, ctag, "FEEDS")
                if rkey not in existing_rels:
                    existing_rels.add(rkey)
                    relations.append({
                        "source_tag": main_panel,
                        "target_tag": ctag,
                        "rel_type": "FEEDS"
                    })

    # F. Map Earthing Components (EARTHED_TO)
    if dt == "EARTHING_LAYOUT":
        eb_tags = [e.get("tag") for e in earthing if "EB" in e.get("tag", "").upper()]
        ep_tags = [e.get("tag") for e in earthing if "EP" in e.get("tag", "").upper()]
        if eb_tags and ep_tags:
            for ep in ep_tags:
                rkey = (ep, eb_tags[0], "EARTHED_TO")
                if rkey not in existing_rels:
                    existing_rels.add(rkey)
                    relations.append({
                        "source_tag": ep,
                        "target_tag": eb_tags[0],
                        "rel_type": "EARTHED_TO"
                    })
